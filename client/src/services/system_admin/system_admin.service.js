...
const getUsersService = async (page = 1, limit = 10, search = "") => {
  try {
    ...
    const response = await axiosInstance.get(
      `/api/systemAdmin/users?${params.toString()}`
    );
...
// Fetch all doctors
const getDoctorsService = async (page = 1, limit = 10, search = "") => {
  try {
    // Construct query parameters
    const params = new URLSearchParams();
    params.append("page", page);
    ...
    const response = await axiosInstance.get(
      `/api/systemAdmin/doctors?${params.toString()}`
    );
...
const getRecentActivitiesService = async (page = 1, limit = 10) => {
  try {
    const response = await axiosInstance.get(
